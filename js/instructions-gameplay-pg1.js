        ...
        if (obj.active) {
            if (checkCollision(player, obj)) {
                ...
                if (objects.length == 0 && passCounter < 3) {
                    runGameSequence("Good Job! Collect all objects " + (3-passCounter) + " more times. Click OK to continue.");
                    spawnUnderstandingCheckObjects();
                    //endGame(false); // UNCOMMENT TO DEBUG
                }
...
$(document).ready( function(){
   ...
   canvas.addEventListener('click', function(event) {
    ...
    // $('#customAlert .custom-alert-close, #customAlert button').click(function() {
    //     $('#customAlert').hide();
    // });
    // Add closeCustomAlert to the global scope
