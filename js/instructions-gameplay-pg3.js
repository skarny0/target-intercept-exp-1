...
async function endGame(advanceRound = false) {
    ...
    if(passCounter < 3 && !understandingCheckFailed){
        understandingCheckFailed    =   false;
        understandingCheckPassed    =   false;
        objects                     =   [];
        
        let trialsLeft = 3 - passCounter;
        // alert("PASS " + trialsLeft + " MORE TO MOVE ON.");
       // await runGameSequence("PASS " + trialsLeft + " MORE TO MOVE ON.");
        // need to offer an option to the abort the experiment entirely
        startGame();
        await runGameSequence("PASS " + trialsLeft + " MORE TO MOVE ON.");
        return;
    }
    else if (understandingCheckFailed && failedCounter < 5) {
        // Load Main Experiment
        //$('#task-main-content').html("");

        understandingCheckFailed    =   false;
        understandingCheckPassed    =   false;
        objects                     =   [];
        let trialsLeft = 5 - failedCounter;
        // alert("YOU CAN FAIL " + trialsLeft + " MORE TIMES UNTIL EXPERIMENT ABORTS.");
        // need to offer an option to the abort the experiment entirely
        startGame();
        await runGameSequence("YOU CAN FAIL " + trialsLeft + " MORE TIMES UNTIL EXPERIMENT ABORTS.");
        return;
    }
    else if (failedCounter >= 5) {
        // Load Main Experiment
        //$('#task-main-content').html("");
        // alert("YOU HAVE FAILED THE COMPREHENSION CHECK 5 TIMES. PLEASE CONTACT THE EXPERIMENTER.");
        await runGameSequence("YOU HAVE FAILED THE COMPREHENSION CHECK 5 TIMES. PLEASE CONTACT THE EXPERIMENTER.");
        return;
    }
}
function gameLoop(timestamp) {
    ...
    //     await runGameSequence("First, Read The Instructions Carefully. After, Click OK to Begin This Task.");
    // }
    ...
    // Regular game loop logic here
    // if understanding check passed
    if      (understandingCheckPassed && !understandingCheckFailed) {
        endGame(false);
        return;
    }
    else if (understandingCheckFailed){
        endGame(false);
        return;
    }
...
function createComposite(settings) {
    ...
    let newObj = {
        ...
        //angle: shapeRotation,
        fill: fillRadius,
        highValue: false, // this is for the understanding check
        active: true,
        spawnX: 0,
        spawnY: 0
    };
...
function drawPlayer() {
    ...
    // console.log("Player Drawn Here:", player.x, player.y);
}
